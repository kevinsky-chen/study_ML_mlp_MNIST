    ...
    def show_images_labels_predictions(images, labels, predictions, start_id, num=10):
        ...
        for i in range(num):
            ...
            if len(predictions) > 0:
                ...
                title += (' (o)' if predictions[i] == labels[start_id + i] else ' (x)')
...
mlp = model()
mlp.layers([784, 256, 128, 10])
mlp.training(training_input_image, training_input_label)
prediction = mlp.testing(testing_input_image, testing_input_label)

preProcessor.show_images_labels_predictions(test_feature, test_label, prediction, 0, 15)   # show出前15個testing結果與label的比對狀況
